...
//int alarm_h_int;
byte alarm_min_dec;//アラームの分設定
byte alarm_min_bcd;
byte alarm_hour_dec;//アラームの時刻設定
byte alarm_hour_bcd;
int alarm_on;
int alarm_stop;
 ...
 */
 ...
 //  //温度センサーの計算
  ...
  if(t_value > 26 && alarm_stop == 0){
  //体温計算
   t_forecast = t_value + 7;
 ...
 if(state == 1){
   ...
   //RTCにアラーム時刻送信
 if(alarm_on == 1){  
   byte alarm_min_dec = alarm_min;
   byte alarm_hour_dec = count;
 ...
 //アラームストップ設定（rtc[1]）
 if(t_forecast > 36.5 && alarm_stop == 0){
   //アラームストップ命令送信
   Wire.beginTransmission(RTC_ADDRESS);
   Wire.write(0x01);
   Wire.write(rtc[1] & 0xf5);
